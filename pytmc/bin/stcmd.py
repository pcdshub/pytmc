...
def build_arg_parser(parser=None):
    ...
    parser.add_argument(
        "-p", "--prefix", type=str, default=None, help="PV prefix for the IOC"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Parse the project only for motor records, skipping other "
            "variables with pytmc pragmas"
        ),
    )

    parser.add_argument("--db-path", type=str, default=".", help="Path for db files")

    parser.add_argument(
        "--dbd", type=str, default=None, help="Path to the IOC dbd file"
    )

    parser.add_argument("--delim", type=str, default=":", help="Preferred PV delimiter")
...
def main(
    ...
    *,
    ...
    jinja_env.filters.update(**jinja_filters(delim=delim, prefix=prefix, name=name))
    ...
    try:
        ...
        if not plc_name:
            raise RuntimeError(
                f"Only single PLC projects supported.  "
        ...
        try:
            ...
            raise RuntimeError(
                f"PLC project {plc_name!r} not found. " f"Projects: {list(by_name)}"
            )
    ...
    if plc.tmc is None:
        ...
        other_records, _ = db.process(plc.tmc, dbd_file=dbd, allow_errors=allow_errors)
        if not other_records:
            logger.info(
                "No additional records from pytmc found in %s", plc.tmc.filename
            )
        else:
            db_filename = f"{plc.filename.stem}.db"
            db_path = pathlib.Path(db_path) / db_filename
            logger.info(
                "Found %d additional records; writing to %s",
                len(other_records),
                db_path,
            )
    ...
    if not allow_errors:
        if not ams_id:
            raise RuntimeError(
                "AMS ID unset. Try --allow-errors if this is " "not an issue."
            )
        if not target_ip:
            raise RuntimeError(
                "IP address unset. Try --allow-errors if this " "is not an issue."
            )
